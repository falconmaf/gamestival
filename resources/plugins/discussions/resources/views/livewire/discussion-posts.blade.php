                ...
                @auth
                    @if ($editingPostId === $post->id)
                        ...
                        <div class="flex items-end justify-end mt-2 space-x-2 text-sm">
                            <x-button color="gray" wire:click="cancelEdit()" class="flex-shrink-0 !{{ config('discussions.styles.rounded') }}">@lang('discussions::messages.words.cancel')</x-button>
                            <x-button wire:click="update({{ $post->id }})" class="flex-shrink-0 !{{ config('discussions.styles.rounded') }}">@lang('discussions::messages.words.save')</x-button>
                        </div>
                    @else
